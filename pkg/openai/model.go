...
func (m *OpenAIChatModel) SetOnSSEReply(handler func(msg model.SSEMessage)) {
	m.Event.OnSSEReplyHandler = handler
}

// OnSSEReply dispatches one message callback when handler is registered.
func (m *OpenAIChatModel) OnSSEReply(msg model.SSEMessage) {
...
func appendOrMergeMessage(messages []model.Message, msg model.Message) []model.Message {
...
func buildMessagesFromChunks(chunks []streamChunk, messages []model.Message) ([]model.Message, string) {
	if messages == nil {
		messages = make([]model.Message, 0, len(chunks))
	}
	...
	for _, chunk := range chunks {
		...
		for _, choice := range chunk.Choices {
			...
			if choice.Delta.ReasoningContent != "" {
				...
				messages = appendOrMergeMessage(messages, msg)
			}
	...
	messages = append(messages, model.Message{Role: "assistant", Kind: model.KindStop, Content: finishReason, ID: lastChunkID, Created: lastChunkCreated, Model: lastChunkModel})
...
func (m *OpenAIChatModel) executeSSE(body io.Reader) ([]model.Message, error) {
	...
	messages, _ := processor.BuildMessages()
	return messages, nil
}
